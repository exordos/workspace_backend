...
| ZB-CATALOG-005 | Report personal-direct catalogs with participant counts other than two and group-direct catalogs with exactly two participants.           | Backend rejects the topology before persisting either the chat or desired assignment. Exactly two personal-direct participants, a singleton group-direct self DM, and group-direct chats with at least three participants remain valid.         |
...
| ZB-CATALOG-007 | Connect an IAM owner whose authenticated Zulip user already exists as an external identity, then attempt to claim the same Zulip identity from another IAM owner. | The first verified account connection merges the external identity into the existing IAM UUID without inheriting another owner's account-scoped stream bindings or unread state. The conflicting owner is rejected fail-closed; email or display name never affects either decision. |
...
| ZB-CONTROL-006 | Recover an empty cursor through a full snapshot containing at least 15,000 assignments with large participant/topic catalogs.              | Snapshot creation stores normalized ordered rows without building one in-process collection; each page reads only its bounded rows, backend RSS remains bounded, and the bridge installs every resource exactly once before advancing the anchor cursor. |
...
| ZB-MSG-017  | Add a mapped user to a provider-backed stream from Workspace, retry the same provider operation, remove the user, retry removal, and add the user again.                                                           | `membership.add`/`membership.remove` use the negotiated `messenger.membership.write` capability and official Zulip subscription API, converge idempotently, and expose any terminal provider failure as a retryable external operation. Pre-join messages are read, post-join messages are unread, and removal immediately revokes Workspace message/event access. |
| ZB-MSG-018  | After a selected channel reaches participant state `ready`, add, remove, and remove-then-readd a Zulip subscriber while leaving the assignment generation unchanged.                                               | Each provider change is detected by the bounded ready-state recheck, reported through the catalog handshake, and converges the Workspace identity, external-chat participants, stream binding, and visible member list without reconnecting the account or polling in an unbounded loop.                                                                        |
...
| ZB-DEPLOY-002 | Replace the backend and provider root images and the UI artifact independently.                                                                                     | Canonical PostgreSQL, S3, provider operational data, and provider identity survive without reinstall or destructive cleanup.                                                   |
...
- [ ] Add a mapped user from Workspace, remove that user, and add the user again.
      Verify the Zulip subscriber list after each step, durable operation state,
      read history boundary, future unread delivery, and immediate access cutoff.
- [ ] On an already converged selected channel, add and remove a subscriber in
      Zulip and then re-add the subscriber. Verify bounded detection and compare
      Zulip subscribers, external-chat participants, Workspace stream bindings,
      and the visible member list after every change.
