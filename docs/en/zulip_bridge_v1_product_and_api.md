...
not hold a PostgreSQL transaction for the lifetime of the session and does not
materialize the complete snapshot as one application-memory or JSONB array.
Snapshot resources are frozen as normalized, ordered PostgreSQL rows and every
HTTP page reads at most the requested limit plus one look-ahead row. This keeps
backend memory bounded when tens of thousands of assignments contain large
participant and topic catalogs. A concurrent create, update, or delete is
either represented in those frozen rows or in the change feed strictly after
the anchor. The bridge installs all pages, then replays changes after the
anchor, and commits the resulting state plus checkpoint atomically. An expired
snapshot token requires starting a new session.
