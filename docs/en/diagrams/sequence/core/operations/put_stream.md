...
![Sequence diagram](diagrams/put_stream.svg)

The source that you can edit: [`put_stream.puml`](diagrams/put_stream.puml).

## The operation

**Method and way:** `PUT /api/workspace/v1/messenger/streams/{stream_uuid}`

**Purpose:** To update the canonical stream's changing fields.
...
Source, identity of the direct stream, privacy and membership are immutable; conflicts give `400`.
...
1. Allow visible stream and verify permissions.
2. Update the canonical fields that are being changed.
3. Add separate immutable outbox events to output
   `topic_membership_policy_rebuild`, `folder_projection` and
...
Tasks: separate `topic_membership_policy_rebuild`, `folder_projection` and
`delivery_snapshot_event`, Each for its own . source outbox event.

The users and providers receive separate projections immutable tasks
The actual ones .`user-stream`/providerTopic work is created only for
affected placements/bindings; one fenced owner writes exact key.

## Public events and WebSocket

`stream.updated` Through the controller ..

## Idempotence, races and time characteristics visible to the client

Unified canonical line and tasks with current source prevent obsolete copy. The caller sees the update immediately, the rest of the projections and events  asynchronously.
