...
![Sequence diagram](diagrams/post_topic_set_default_action.svg)

The source that you can edit: [`post_topic_set_default_action.puml`](diagrams/post_topic_set_default_action.puml).

## The operation

**Method and way:** `POST /api/workspace/v1/messenger/stream_topics/{topic_uuid}/actions/set_default/invoke`

**Purpose:** Set the topic as the default stream topic.
...
2. Set it up STREAM.default_topic_uuid.
3. When changing , add separate immutable outbox events for
   `topic_state_projection` and `delivery_snapshot_event`.
...
Tasks: separate `topic_state_projection` and `delivery_snapshot_event`, each
For his own source outbox event.

The old and new user-stream/user-topic projections are updated separately
immutable tasks exact scopes With one fenced owner on key. Topic worker shared
rows Does not write; scopes results visible eventual-consistently.

## Public events and WebSocket

The controller delivers the fixed ready lines.

## Idempotence, races and time characteristics visible to the client

The state setting is idempotent, the external key is indexed..
