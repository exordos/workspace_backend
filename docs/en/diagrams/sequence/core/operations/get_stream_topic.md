...
![Sequence diagram](diagrams/get_stream_topic.svg)

The source that you can edit: [`get_stream_topic.puml`](diagrams/get_stream_topic.puml).

## The operation

**Method and way:** `GET /api/workspace/v1/messenger/stream_topics/{topic_uuid}`

**Purpose:** To provide one visible representation of the topic.

## A public request

Path: `topic_uuid = 4ec0b996-b778-45f8-8ef4-ef863be0c047`; without body.
...
1. Allow indexed binding of the current user's topic, attach canonical TOPIC and ready state only with one-to-one and many-to-one links».
