...
![Sequence diagram](diagrams/post_stream_topics_create.svg)

The source that you can edit: [`post_stream_topics_create.puml`](diagrams/post_stream_topics_create.puml).

## The operation

**Method and way:** `POST /api/workspace/v1/messenger/stream_topics/`

**Purpose:** To create one canonical topic.
...
1. Check the stream is bound.
2. Insert TOPIC and USER_TOPIC_BINDING creator.
3. Add separate immutable outbox events to output
   `topic_membership_policy_rebuild`, `topic_state_projection` and
   `delivery_snapshot_event` tasks.

Affected state: TOPIC, creator binding and transactional outbox.

## Typed tasks and background performers

The tasks: `topic_membership_policy_rebuild`,
`topic_state_projection` and `delivery_snapshot_event`, each for its own
source outbox event.

For each user `USER_TOPIC_BINDING` a separate user interface is created immutable
task scope `user-topic`; placement/message work The new topic uses scope
`topic`. One fenced owner writes the exact key, different scopes parallel.

## Public events and WebSocket

`topic.created` Through the controller ..

## Idempotence, races and time characteristics visible to the client

The canonical line is unique, the topic and user bindings are unique; the creator sees the result immediately, other users and events  asynchronously.
