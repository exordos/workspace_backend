# Repeal the copy of the external bridge
...
Can 't undo the active certificate generation.

![Sequence diagram](diagrams/post_external_bridge_instance_revoke.svg)

[The source that you can edit PlantUML](diagrams/post_external_bridge_instance_revoke.puml)
...
| --- | --- |
| `403` | No `workspace.external_bridge_instance.revoke` permission or resource is outside authorized area. |
...
2. Block the bridge instance; apply the `revoke` state transition and revision/generation rule; write an unchangeable domain outbox entry; record the transaction.
