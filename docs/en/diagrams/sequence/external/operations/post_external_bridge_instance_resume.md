# Restarting the work of the external bridge
...
Restore a suspended but not revoked identity.

![Sequence diagram](diagrams/post_external_bridge_instance_resume.svg)

[The source that you can edit PlantUML](diagrams/post_external_bridge_instance_resume.puml)
...
| --- | --- |
| `403` | No `workspace.external_bridge_instance.resume` permission or resource is outside authorized area. |
...
2. Block the bridge instance; apply the `resume` state transition and revision/generation rule; write an unchangeable domain outbox entry; record the transaction.
