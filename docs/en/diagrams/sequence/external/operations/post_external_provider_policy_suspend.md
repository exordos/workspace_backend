# Suspending the policy of the external provider
...
Suspend provider view across the realm after policy check.

![Sequence diagram](diagrams/post_external_provider_policy_suspend.svg)

[The source that you can edit PlantUML](diagrams/post_external_provider_policy_suspend.puml)
...
| --- | --- |
| `403` | No `workspace.external_provider_policy.suspend` permission or resource is outside authorized area. |
...
2. Block policy; set `emergency_suspended=true`; enlarge revision; atomically add desired state and immutable outbox.
