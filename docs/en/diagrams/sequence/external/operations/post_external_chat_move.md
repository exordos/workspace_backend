# Move the external chat projection
...
Automatically move the selected projection to another project, keeping the stable UUID resources.

![Sequence diagram](diagrams/post_external_chat_move.svg)

[The source that you can edit PlantUML](diagrams/post_external_chat_move.puml)
...
| `409` | `ExternalProjectionMoveConflictError`, While the read-provider delivery is being executed. |
...
2. Block chat revision and old/new project assignment; reject parallel delivery of read provider; atomically add canonical move transition, desired state and immutable outbox.
...
Typed tasks: separate `topic_membership_policy_rebuild` for old and
new topic and `delivery_snapshot_event` for external-chat state/event, each with
with its source outbox event; request path does not perform fan-out or aggregate scanning.
...
Consistency visible to the client: destination transition may remain pending while projections of the old/new project converge; stable public UUID chats/entities are maintained.
