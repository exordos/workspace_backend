...
![Sequence diagram](diagrams/get_folder_items_list.svg)

[The source that you can edit PlantUML](diagrams/get_folder_items_list.puml)

## Purpose and public contract

List the current user folder items.
...
The object is not copied by the current contract, but by the current contract.
Automatic elements  restored materialized projection:
`USER_STREAM_BINDING` is created/updated/deleted through
transactional outbox and a separate immutable task with unique `outbox_event_uuid`, then worker
Idimpotently materializes the active `USER_STREAM_BINDING`, connected to
...
`Channels` — Only with .`STREAM.private = false`Worker also updates the ready-made
The aggregates `unread_count`/`mention_count` in `USER_FOLDER_BINDING`. GET
performs only simple indexed connections without `COUNT` during
The request.
...
The page items reads normalized `FOLDER_ITEM` and one by one
The indexed many-to-one join takes the ready counters from
`USER_STREAM_BINDING`. There's no N+1 and custom SQL. normalized rows
are source of truth for read-only `folder_items_snapshot`; this GET is not
It 's not rebuilding it , it 's just doing it . `folder_projection`.
