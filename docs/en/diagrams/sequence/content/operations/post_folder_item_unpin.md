...
![Sequence diagram](diagrams/post_folder_item_unpin.svg)

[The source that you can edit PlantUML](diagrams/post_folder_item_unpin.puml)
...
Unmounting changes only the personal placement layer of the element and does not change
The system folder rule or automatic membership.
`FOLDER_ITEM` remains supported by the worker restoring projection from
active `USER_STREAM_BINDING` and canonical `STREAM` with
`is_archived = false`: `All chats` Includes all available streams,
`Personal` — only flows from `STREAM.private = true`, `Channels`  only from
`STREAM.private = false`. Her idea of reading uses simple
Indexed connections without `COUNT` at query time.

## Synchronous path API

1. Find and block a folder item in the specified region.
2. Set `pinned_at` to `null`.
3. Add a recording to the outbox `folder_item.unpin`.
...
The response REST immediately contains `pinned_at: null`; embedded read-only snapshot folders, counters and WebSocket events may lag before completion `folder_projection`.
