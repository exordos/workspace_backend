...
![Sequence diagram](diagrams/get_user.svg)

[The source that you can edit PlantUML](diagrams/get_user.puml)

## Purpose and public contract

Get one materialized user Workspace; the request of the own user first updates the projection from IAM.
...
UUID The error UUID or IAM is handled by the common error boundary.
...
2. If UUID is owned by the current user, synchronously update the identity fields from IAM; otherwise, perform only a projection search.
3. Read and sanitize user canonical line.
4. Return it. GET does not create public outbox records, tasks or events.
