...
messages by `(latest reaction time, uuid)` descending, so a new reaction moves
an older message to the front. The marker is resolved in the same project,
author, and visibility scope even if its last reaction was removed between page
requests, so pagination can continue from a previously returned row.
...
does not add provider-specific collection routes. The full sanitized resource
shapes, ETag rules, action permissions, and realtime events are defined in
...
query. Eligibility and the latest-reaction key are maintained from canonical
reaction rows on the reaction write path, independently of the bounded
`reaction_users` snapshot. Therefore messages with a large reaction group or
historical `reaction_users: {}` remain eligible. The latest-reaction key is
monotonic while reactions are removed, which keeps issued pagination markers
stable; a later new reaction advances it.
...
filter or sort parameters: order is fixed to `(latest reaction time, uuid)`
descending.
...
surface and contains no Provider, Mail, or Calendar browser routes. The private
Provider contract is maintained separately in
[`workspace_provider_api_v1.yaml`](workspace_provider_api_v1.yaml).
