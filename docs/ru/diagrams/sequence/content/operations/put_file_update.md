...
![Диаграмма последовательности](diagrams/put_file_update.svg)

[Редактируемый исходник PlantUML](diagrams/put_file_update.puml)

## Назначение и публичный контракт

Обновить метаданные файла владельца; байты остаются неизменяемыми.
...
Обновлять может только владелец. Неверные метаданные возвращают `400`; недоступный UUID или UUID другого владельца не раскрывается. Текущий контракт не определяет предусловие ETag для обновления метаданных.
...
1. Найти файл владельца.
2. Проверить изменяемые метаданные и сохранить контракт неизменяемых байтов/хеша.
3. Обновить канонические метаданные/сопутствующий файл по необходимости.
4. Добавить в outbox неизменяемую запись `file.updated` в транзакции БД и зафиксировать её.
5. Вернуть санитизированные метаданные.

## Outbox, типизированные задачи, воркер и работа в реальном времени

Снимок участников ACL и сканирование сообщений не выполняются. Динамический доступ к потоку по-прежнему определяется привязками.
...
`outbox_event_uuid`. Воркер идемпотентно записывает готовое `file.updated`, а
диспетчер отправляет, повторяет или воспроизводит его.

## Идемпотентность, ключи и гонки

Область владельца предотвращает обновления между пользователями. Конкурирующие записи метаданных разрешаются сериализацией строки БД; текущий контракт не обещает оптимистический ETag.
