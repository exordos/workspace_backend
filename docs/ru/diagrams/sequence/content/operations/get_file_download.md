...
![Диаграмма последовательности](diagrams/get_file_download.svg)

[Редактируемый исходник PlantUML](diagrams/get_file_download.puml)

## Назначение и публичный контракт

Скачать неизменяемые байты видимого файла.
...
`200`: необработанные байты, не JSON. Заголовки: сохранённый `Content-Type`, вложение `Content-Disposition`, строгий `ETag: "<hash>"` и поведение private/no-cache.
...
1. Аутентифицировать запрос и найти метаданные по UUID.
2. Проверить публичный ACL или текущее индексированное членство в потоке.
3. Прочитать неизменяемые бинарные данные из настроенного хранилища.
4. Передать байты с заголовками целостности и вложения.
