...
![Диаграмма последовательности](diagrams/get_users_list.svg)

[Редактируемый исходник PlantUML](diagrams/get_users_list.puml)

## Назначение и публичный контракт

Перечислить уже материализованных пользователей Workspace; глобальный маршрут не выполняет опережающий импорт идентичностей IAM.
...
2. Прочитать материализованных пользователей через индексированные фильтры/пагинацию.
3. Санитизировать метаданные внешнего провайдера.
4. Вернуть список без импорта пользователей IAM.
