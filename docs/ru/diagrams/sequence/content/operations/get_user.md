...
![Диаграмма последовательности](diagrams/get_user.svg)

[Редактируемый исходник PlantUML](diagrams/get_user.puml)

## Назначение и публичный контракт

Получить одного материализованного пользователя Workspace; запрос собственного пользователя сначала обновляет проекцию из IAM.
...
UUID другого пользователя, ещё не материализованного его собственной активностью, возвращается как не найден. Неверный UUID или состояние IAM обрабатывается общей границей ошибок.
...
2. Если UUID принадлежит текущему пользователю, синхронно обновить поля идентичности из IAM; иначе выполнить только поиск по проекции.
3. Прочитать и санитизировать каноническую строку пользователя.
4. Вернуть его. GET не создаёт публичных записей outbox, задач или событий.
