...
![Диаграмма последовательности](diagrams/get_folder_item.svg)

[Редактируемый исходник PlantUML](diagrams/get_folder_item.puml)

## Назначение и публичный контракт

Получить один элемент папки текущего пользователя.
...
объектом (в текущем контракте — поток), не копируя его. Автоматический элемент
системной папки — восстанавливаемая материализованная проекция, которую воркер
идемпотентно поддерживает из активной `USER_STREAM_BINDING` и канонической
...
только с `STREAM.private = false`. Этот GET только соединяет индексированные
строки; `COUNT` во время запроса и обход сообщений запрещены.
...
Публичный item читает одну normalized `FOLDER_ITEM` и одну готовую
`USER_STREAM_BINDING` по индексу; это тот же source of truth, из которого
`folder_projection` строит read-only `USER_FOLDER_BINDING.folder_items_snapshot`.
Этот GET не исправляет и не перестраивает snapshot.
