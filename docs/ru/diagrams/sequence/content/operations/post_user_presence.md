...
![Диаграмма последовательности](diagrams/post_user_presence.svg)

[Редактируемый исходник PlantUML](diagrams/post_user_presence.puml)

## Назначение и публичный контракт

Обновить собственные присутствие и отметку активности аутентифицированного пользователя.
...
Принимается только собственный UUID аутентифицированного пользователя. `status` принимает `active|idle|offline|do_not_disturb`; `emoji` и `text` можно опустить, чтобы сохранить прежние значения, либо передать как `null` для очистки.
...
1. Проверить собственный UUID.
2. Проверить `status` и необязательные поля.
3. Обновить поле `status` и установить `last_ping_at=now`.
4. Добавить в outbox неизменяемую запись `user.presence_changed`.
5. Зафиксировать транзакцию и вернуть полный снимок пользователя.

## Outbox, типизированные задачи, воркер и работа в реальном времени

API не обновляет синхронно событие каждого получателя. Перевод неактивных пользователей в `offline` — отдельная ответственность воркера.
...
Каноническая строка пользователя сериализует конкурирующие записи присутствия; побеждает последнее зафиксированное значение `status`. Каждое outbox-событие выводит отдельную immutable task; идемпотентный обработчик читает актуальное исходное состояние, поэтому повтор той же task не выдаёт устаревший снимок за текущий.

Для mapped Zulip identity Bridge последовательно доставляет Workspace-origin и
Zulip-origin presence/status changes. Последнее confirmed изменение побеждает;
`origin`/`causation_uuid` только подавляют собственное echo и не задают приоритет
системы. Public request/response shape не меняется.
