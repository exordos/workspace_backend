...
![Диаграмма последовательности](diagrams/post_folder_item_unpin.svg)

[Редактируемый исходник PlantUML](diagrams/post_folder_item_unpin.puml)

## Назначение и публичный контракт

Открепить элемент папки текущего пользователя.
...
Открепление меняет только персональный слой размещения (placement) элемента и не меняет
правило системной папки или автоматическое членство. Сам автоматический
`FOLDER_ITEM` остаётся поддерживаемой воркером восстанавливаемой проекцией из
активной `USER_STREAM_BINDING` и канонической `STREAM` с
...
`STREAM.private = false`. Её представление чтения использует простые
индексированные соединения без `COUNT` во время запроса.

## Синхронный путь API

1. Найти и заблокировать элемент папки в заданной области.
2. Установить `pinned_at` в `null`.
3. Добавить в outbox неизменяемую запись `folder_item.unpin`.
...
Ответ REST сразу содержит `pinned_at: null`; вложенный read-only snapshot папки, счётчики и WebSocket event могут отставать до завершения `folder_projection`.
