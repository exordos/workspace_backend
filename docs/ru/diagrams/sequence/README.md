[← Главный индекс документации](../../index.md) · [Раздел фоновых потоков](worker_flows/README.md) · [Действующий API-контракт](../../workspace_api.md)

# Диаграммы последовательностей операций Workspace API

Статус: **завершённое проектное предложение, начатое с документации и сопоставленное с действующим публичным контрактом**.

Этот каталог покрывает все **109 семантически различных HTTP-операций «метод + путь»**:
108 операций контроллеров из сгенерированного OpenAPI и одна каноническая конечная точка промежуточного ПО
`GET /api/workspace/v1/messenger/server_settings`. Тот же обработчик промежуточного ПО принимает
вариант с завершающим слешем и тем же прямым `200`, без перенаправления; это псевдоним
одной операции, а не отдельная конечная точка. Дополнительно описан отдельный
поток времени исполнения WebSocket Events. Для каждой HTTP-операции существуют
самостоятельные Markdown-описание, редактируемый PlantUML-исходник и
локально отрендеренный SVG.

Файлы проектного предложения объясняют целевой поток транзакции RestAlchemy, транзакционного outbox, задач, воркера (worker) и событий, но не меняют действующий публичный API.

Принятые target compatibility rules для всех документов операций:

- resource-list pagination: отсутствующий/`0` `page_limit` => `100`, `1..500`
  точно, отрицательное/нецелое/`>500` => HTTP `400`, unlimited отсутствует;
- `2xx`/`201` означает commit primary mutation и immediate read-your-write для
  автора, но recipient/history/counters/snapshots/events строятся асинхронно;
  около секунды — SLO intent, не строгая гарантия;
- worker фиксирует projection update и все соответствующие durable ready event
  rows в одной DB transaction. Dispatcher только доставляет at-least-once;
  reconnect использует cursor replay без gap и client dedupe по event UUID.

Это сознательное observable behavior change. Клиенты полного экспорта обязаны
читать страницы до отсутствия следующего marker; release note должен описывать
его вместе с асинхронной видимостью.

## Индексы семейств

| Семейство | Покрытие | Индекс |
| --- | ---: | --- |
| Основной Messenger | 45 HTTP-операций | [`core/README.md`](core/README.md) |
| Контент, пользователи и клиентское состояние Workspace | 35 HTTP-операций | [`content/README.md`](content/README.md) |
| События, внешние интеграции и время исполнения | 29 HTTP-операций + WebSocket | [`external/README.md`](external/README.md) |
| Потоки воркера, outbox и проекций | 7 потоков без HTTP | [`worker_flows/README.md`](worker_flows/README.md) |

## Полная матрица покрытия

### Индексы маршрутов

| Метод + путь | Markdown операции | PlantUML | SVG |
| --- | --- | --- | --- |
...
| Точка входа времени исполнения | Markdown | PlantUML | SVG |
| --- | --- | --- | --- |
| `GET /api/workspace/v1/events/ws` (переход на WebSocket и поток времени исполнения) | [`websocket_events.md`](external/operations/websocket_events.md) | [`websocket_events.puml`](external/operations/diagrams/websocket_events.puml) | [`websocket_events.svg`](external/operations/diagrams/websocket_events.svg) |

## Фоновые потоки воркера

| Поток | Markdown | PlantUML | SVG |
| --- | --- | --- | --- |
| Архитектура воркера | [`worker_architecture.md`](worker_flows/worker_architecture.md) | [`worker_architecture.puml`](worker_flows/diagrams/worker_architecture.puml) | [`worker_architecture.svg`](worker_flows/diagrams/worker_architecture.svg) |
| Распределение получателей (fan-out) | [`task_fanout.md`](worker_flows/task_fanout.md) | [`task_fanout.puml`](worker_flows/diagrams/task_fanout.puml) | [`task_fanout.svg`](worker_flows/diagrams/task_fanout.svg) |
| Контент и упоминания | [`task_content_mentions.md`](worker_flows/task_content_mentions.md) | [`task_content_mentions.puml`](worker_flows/diagrams/task_content_mentions.puml) | [`task_content_mentions.svg`](worker_flows/diagrams/task_content_mentions.svg) |
| Снимки реакций | [`task_reaction_snapshot.md`](worker_flows/task_reaction_snapshot.md) | [`task_reaction_snapshot.puml`](worker_flows/diagrams/task_reaction_snapshot.puml) | [`task_reaction_snapshot.svg`](worker_flows/diagrams/task_reaction_snapshot.svg) |
| Чтение и счётчики контейнеров | [`task_read_counters.md`](worker_flows/task_read_counters.md) | [`task_read_counters.puml`](worker_flows/diagrams/task_read_counters.puml) | [`task_read_counters.svg`](worker_flows/diagrams/task_read_counters.svg) |
| Снимки доставки и готовые события | [`task_delivery_snapshot_event.md`](worker_flows/task_delivery_snapshot_event.md) | [`task_delivery_snapshot_event.puml`](worker_flows/diagrams/task_delivery_snapshot_event.puml) | [`task_delivery_snapshot_event.svg`](worker_flows/diagrams/task_delivery_snapshot_event.svg) |
| Перестроение членства и политики темы | [`task_topic_membership_policy_rebuild.md`](worker_flows/task_topic_membership_policy_rebuild.md) | [`task_topic_membership_policy_rebuild.puml`](worker_flows/diagrams/task_topic_membership_policy_rebuild.puml) | [`task_topic_membership_policy_rebuild.svg`](worker_flows/diagrams/task_topic_membership_policy_rebuild.svg) |
| Проекция канонического состояния темы | [`task_topic_membership_policy_rebuild.md`](worker_flows/task_topic_membership_policy_rebuild.md#topic_state_projection) | [`task_topic_membership_policy_rebuild.puml`](worker_flows/diagrams/task_topic_membership_policy_rebuild.puml) | [`task_topic_membership_policy_rebuild.svg`](worker_flows/diagrams/task_topic_membership_policy_rebuild.svg) |
| Runbook миграции и выпуска | [`migration_release_runbook.md`](worker_flows/migration_release_runbook.md) | [`migration_release_runbook.puml`](worker_flows/diagrams/migration_release_runbook.puml) | [`migration_release_runbook.svg`](worker_flows/diagrams/migration_release_runbook.svg) |

## Навигация и границы

В каждом Markdown-файле операции, семейства или воркера одинаковая навигационная строка находится сверху и снизу. Публичный контракт определяется только [`workspace_api.md`](../../workspace_api.md); спецификации последовательностей являются проектным предложением, начатым с документации, и не разрешают реализацию в производственном коде.
