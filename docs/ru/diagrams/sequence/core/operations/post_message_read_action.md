...
![Диаграмма последовательности](diagrams/post_message_read_action.svg)

Редактируемый исходник: [`post_message_read_action.puml`](diagrams/post_message_read_action.puml).

## Операция

**Метод и путь:** `POST /api/workspace/v1/messenger/messages/{message_uuid}/actions/read/invoke`

**Назначение:** Отметить одно сообщение прочитанным для текущего пользователя.
...
Публичный `uuid` и route ID равны `MESSAGE_PLACEMENT.uuid`; canonical `MESSAGE.uuid` и `binding_uuid` скрыты. Placement однозначно выбирает `USER_MESSAGE_STATE (project,user,placement)`, а action синхронно проверяет active membership и generation.

## Синхронная транзакция

1. Разрешить публичный placement UUID и доступ текущего пользователя.
2. Установить уникальное значение USER_MESSAGE_STATE.read.
3. Только при изменении добавить отдельное immutable outbox event для каждой
   выводимой task фактической области.
