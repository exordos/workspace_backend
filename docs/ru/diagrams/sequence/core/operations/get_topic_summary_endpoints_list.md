...
![Диаграмма последовательности](diagrams/get_topic_summary_endpoints_list.svg)

Редактируемый исходник: [`get_topic_summary_endpoints_list.puml`](diagrams/get_topic_summary_endpoints_list.puml).

## Операция

**Метод и путь:** `GET /api/workspace/v1/messenger/topic_summary_endpoints/`

**Назначение:** Получить список глобальных OpenAI-совместимых конечных точек сводок в порядке приоритета и UUID.

## Публичный запрос

Без тела; фильтры коллекции и переопределение порядка не принимаются.

Текущая конечная точка не принимает `page_limit` и читает весь реестр конечных
точек — это current gap. Target принимает `page_limit` и `page_marker`:
отсутствие/`0` даёт `100`, `1..500` принимается точно, отрицательное,
нецелочисленное и `>500` даёт HTTP `400` без clamp. JSON остаётся массивом;
страничность передаётся стандартными `X-Pagination-Limit` и
`X-Pagination-Marker`, поэтому новые JSON-поля не появляются.
