...
![Диаграмма последовательности](diagrams/get_stream_binding.svg)

Редактируемый исходник: [`get_stream_binding.puml`](diagrams/get_stream_binding.puml).

## Операция

**Метод и путь:** `GET /api/workspace/v1/messenger/stream_bindings/{binding_uuid}`

**Назначение:** Получить одну видимую привязку потока.

## Публичный запрос

Путь: `binding_uuid = 3195a887-da5d-440b-bdf8-0d3d995a9e01`; без тела.
...
1. Восстановить привязку по UUID, проверить членство и роль наблюдателя, сериализовать скалярные UUID. Физические готовые счётчики не расширяют текущий публичный JSON привязки.
