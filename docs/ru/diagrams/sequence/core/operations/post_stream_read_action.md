...
![Диаграмма последовательности](diagrams/post_stream_read_action.svg)

Редактируемый исходник: [`post_stream_read_action.puml`](diagrams/post_stream_read_action.puml).

## Операция

**Метод и путь:** `POST /api/workspace/v1/messenger/streams/{stream_uuid}/actions/read/invoke`

**Назначение:** Отметить сообщения потока прочитанными для текущего пользователя.
...
Затрагиваемое состояние: USER_MESSAGE_STATE и transactional outbox; агрегат никогда не хранится в привязке сообщения.

## Типизированные задачи и фоновый исполнитель

Задачи: отдельные immutable `read_counters` tasks для `user-stream` и
`user-topic`, а также `folder_projection` для `user-folder`; каждая
соответствует своему source outbox event, coalescing отсутствует.

Fenced owners exact scopes `user-stream`, `user-topic` и `user-folder`
обновляют готовые счётчики/snapshot; topic worker shared rows не пишет. Atomic delta
требует exactly-once guard по `outbox_event_uuid`, иначе выполняется
recompute/write. Tasks используют retry/backoff, DLQ/reaper.

## Публичные события и WebSocket

`stream.read` и обновления контейнеров.

## Идемпотентность, гонки и видимые клиенту временные характеристики

Повторная отметка прочтения идемпотентна; состояние прочтения меняется сразу, счётчики и события — асинхронно.
