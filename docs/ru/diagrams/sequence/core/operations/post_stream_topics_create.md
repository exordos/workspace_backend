...
![Диаграмма последовательности](diagrams/post_stream_topics_create.svg)

Редактируемый исходник: [`post_stream_topics_create.puml`](diagrams/post_stream_topics_create.puml).

## Операция

**Метод и путь:** `POST /api/workspace/v1/messenger/stream_topics/`

**Назначение:** Создать одну каноническую тему.
...
1. Проверить привязку потока.
2. Вставить TOPIC и USER_TOPIC_BINDING создателя.
3. Добавить отдельные immutable outbox events для выводимых
   `topic_membership_policy_rebuild`, `topic_state_projection` и
   `delivery_snapshot_event` tasks.

Затрагиваемое состояние: TOPIC, привязка создателя и transactional outbox.

## Типизированные задачи и фоновый исполнитель

Задачи: отдельные `topic_membership_policy_rebuild`,
`topic_state_projection` и `delivery_snapshot_event`, каждая для собственного
source outbox event.

Для каждой пользовательской `USER_TOPIC_BINDING` создаётся отдельная immutable
task scope `user-topic`; placement/message work новой темы использует scope
`topic`. Один fenced owner пишет exact key, разные scopes параллельны.

## Публичные события и WebSocket

`topic.created` через диспетчер.

## Идемпотентность, гонки и видимые клиенту временные характеристики

Каноническая строка единственна, привязки темы и пользователя уникальны; создатель видит результат сразу, остальные пользователи и события — асинхронно.
