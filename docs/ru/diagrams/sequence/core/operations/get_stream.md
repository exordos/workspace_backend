...
![Диаграмма последовательности](diagrams/get_stream.svg)

Редактируемый исходник: [`get_stream.puml`](diagrams/get_stream.puml).

## Операция

**Метод и путь:** `GET /api/workspace/v1/messenger/streams/{stream_uuid}`

**Назначение:** Получить одну видимую пользователю строку потока.

## Публичный запрос

Путь: `stream_uuid = 75309057-419c-4b12-a7c1-3932429ec4a6`; без тела.
...
1. Разрешить индексированную привязку `(project,user,stream)`, присоединить каноническую STREAM и готовые агрегаты привязки. Ссылки UUID остаются скалярным JSON.
