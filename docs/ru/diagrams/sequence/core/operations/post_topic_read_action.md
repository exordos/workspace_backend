...
![Диаграмма последовательности](diagrams/post_topic_read_action.svg)

Редактируемый исходник: [`post_topic_read_action.puml`](diagrams/post_topic_read_action.puml).

## Операция

**Метод и путь:** `POST /api/workspace/v1/messenger/stream_topics/{topic_uuid}/actions/read/invoke`

**Назначение:** Отметить сообщения темы прочитанными для текущего пользователя.
...
1. Разрешить область темы.
2. Установить маркеры прочтения USER_MESSAGE_STATE.
...
Задачи: отдельные immutable `read_counters` tasks для exact scopes
`user-topic`/`user-stream` и `folder_projection` для `user-folder`; coalescing
отсутствует.

Fenced owners exact scopes обновляют соответствующие shared bindings; topic
worker их не пишет. Atomic delta требует exactly-once guard по
`outbox_event_uuid`, иначе выполняется recompute/write. Tasks используют
retry/backoff, DLQ/reaper.

## Публичные события и WebSocket

`topic.read` и обновления контейнеров. Диспетчер доставляет зафиксированные готовые строки.

## Идемпотентность, гонки и видимые клиенту временные характеристики

Операция идемпотентна; в клиентском пути нет агрегирования. Вызывающий видит состояние сразу, производные проекции и события — асинхронно.
