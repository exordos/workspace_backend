...
![Диаграмма последовательности](diagrams/put_stream.svg)

Редактируемый исходник: [`put_stream.puml`](diagrams/put_stream.puml).

## Операция

**Метод и путь:** `PUT /api/workspace/v1/messenger/streams/{stream_uuid}`

**Назначение:** Обновить изменяемые поля канонического потока.
...
Источник, идентичность прямого потока, приватность и членство неизменяемы; конфликты дают `400`.
...
1. Разрешить видимый поток и проверить права.
2. Обновить изменяемые канонические поля.
3. Добавить отдельные immutable outbox events для выводимых
   `topic_membership_policy_rebuild`, `folder_projection` и
...
Задачи: отдельные `topic_membership_policy_rebuild`, `folder_projection` и
`delivery_snapshot_event`, каждая для собственного source outbox event.

Проекции пользователей и провайдера получают отдельные immutable tasks
фактических `user-stream`/provider scopes. Topic work создаётся только для
затронутых placements/bindings; один fenced owner пишет exact key.

## Публичные события и WebSocket

`stream.updated` через диспетчер.

## Идемпотентность, гонки и видимые клиенту временные характеристики

Единая каноническая строка и задачи с актуальным источником предотвращают устаревшую копию. Вызывающий видит обновление сразу, остальные проекции и события — асинхронно.
