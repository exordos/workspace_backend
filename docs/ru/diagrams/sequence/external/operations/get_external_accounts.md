# Список внешних учётных записей
...
Перечислить глобальные для области (realm) внешние учётные записи, принадлежащие текущему пользователю IAM.

![Диаграмма последовательностей](diagrams/get_external_accounts.svg)

[Редактируемый исходник PlantUML](diagrams/get_external_accounts.puml)
...
- `status`
- `page_limit`
- `page_marker` (UUID последней учётной записи)
