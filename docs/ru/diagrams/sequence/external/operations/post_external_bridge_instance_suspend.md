# Приостановка экземпляра внешнего моста
...
Немедленно приостановить идентичность без отзыва её поколения.

![Диаграмма последовательностей](diagrams/post_external_bridge_instance_suspend.svg)

[Редактируемый исходник PlantUML](diagrams/post_external_bridge_instance_suspend.puml)
...
| --- | --- |
| `403` | Отсутствует разрешение `workspace.external_bridge_instance.suspend` или ресурс находится вне авторизованной области. |
...
2. Заблокировать экземпляр моста; применить переход состояния `suspend` и правило ревизии/поколения; записать неизменяемую запись доменного outbox; зафиксировать транзакцию.
