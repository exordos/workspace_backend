...
Перечислить устойчивую работу владельца, связанную с провайдером.

![Диаграмма последовательностей](diagrams/get_external_operations.svg)

[Редактируемый исходник PlantUML](diagrams/get_external_operations.puml)

## Запрос

Контракт параметров запроса:

- Необязательный `external_account_uuid`
- Необязательный `status`
- `page_limit`
- `page_marker` (UUID последней операции)
