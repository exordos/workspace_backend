# Перемещение проекции внешнего чата
...
Атомарно переместить выбранную проекцию в другой проект, сохранив стабильные UUID ресурсов.

![Диаграмма последовательностей](diagrams/post_external_chat_move.svg)

[Редактируемый исходник PlantUML](diagrams/post_external_chat_move.puml)
...
| `409` | `ExternalProjectionMoveConflictError`, пока выполняется доставка чтения провайдера. |
...
2. Заблокировать ревизию чата и старое/новое назначение проекта; отклонить параллельную доставку чтения провайдера; атомарно добавить канонический переход перемещения, желаемое состояние и неизменяемый outbox.
...
Типизированные задачи: отдельные `topic_membership_policy_rebuild` для старой и
новой тем и `delivery_snapshot_event` для external-chat state/event, каждая со
своим source outbox event; request path не выполняет fan-out или сканирование агрегатов.
...
Согласованность, видимая клиенту: переход назначения может оставаться ожидающим, пока сходятся проекции старого/нового проекта; стабильные публичные UUID чата/сущностей сохраняются.
