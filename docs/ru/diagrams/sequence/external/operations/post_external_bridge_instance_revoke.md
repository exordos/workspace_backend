# Отзыв экземпляра внешнего моста
...
Необратимо отозвать активное поколение сертификата.

![Диаграмма последовательностей](diagrams/post_external_bridge_instance_revoke.svg)

[Редактируемый исходник PlantUML](diagrams/post_external_bridge_instance_revoke.puml)
...
| --- | --- |
| `403` | Отсутствует разрешение `workspace.external_bridge_instance.revoke` или ресурс находится вне авторизованной области. |
...
2. Заблокировать экземпляр моста; применить переход состояния `revoke` и правило ревизии/поколения; записать неизменяемую запись доменного outbox; зафиксировать транзакцию.
