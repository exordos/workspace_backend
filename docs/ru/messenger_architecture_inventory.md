...
Путь захвата проходит по взвешенному циклу полос: четыре слота fan-out, два
слота интерактивных read-действий, один слот реакций, один слот фонового
read-state и два слота остальных фоновых задач. На каждом шаге выбирается
самая старая доступная задача предпочтительной полосы среди всех проектов, а
если полоса пуста — самая старая доступная задача любой полосы. Поиск проекта и
финальный захват используют одинаковые условия по предшественникам, retry и
scope lease. Ограниченные выборки и частичные индексы активных задач не дают
PostgreSQL сортировать всю историю, а scope leases, advisory locks проекта и
fencing tokens обеспечивают безопасную параллельную работу worker-процессов.
