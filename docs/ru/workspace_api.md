...
| `encryption` | булевой | Нет | неизменяемый | Видимый клиенту признак зашифрованного чата; по умолчанию `false` и не зависит от `private`. |
| `current_encryption_key` | объект или `null` | условно | управляется ротацией | Обязателен при создании стрима с `encryption: true`. Содержит только `key_uuid` и непрозрачный публичный `public_key`; владелец стрима меняет его сообщением `key_announced`. |
...
Запрос на создание зашифрованного стрима:
...
`encryption` и начальный ключ нельзя изменить через `PUT`. Публичный ключ
является непрозрачными данными клиента; backend не хранит приватные ключи и не
выполняет шифрование. Отправка `key_announced` атомарно сохраняет сообщение и
заменяет `current_encryption_key`; такую ротацию может выполнять только
канонический владелец стрима.
...
`POST /api/workspace/v1/messenger/messages/` проверяет текущее членство в
PostgreSQL-стриме и в транзакции сохраняет каноническое сообщение, флаги, один
общий снимок аудитории получателей и компактные события message/topic/stream.
Отдельная строка канонического события для каждого получателя не создаётся.
Чтение остаётся ограниченным текущим пользователем IAM и сохраняет прежний ответ.

API принимает шесть фиксированных форм payload. Неизвестные поля отклоняются
для каждого kind. Все шесть видов допустимы как в обычных, так и в
зашифрованных стримах; `stream.encryption` — видимая клиенту метаинформация, а
не фильтр видов сообщений.
...
Значения `content` и `public_key` — непустые непрозрачные строки длиной не
более 40 000 символов; идентификаторы и имена устройств ограничены 255
символами. Backend не проверяет Base64, не выбирает текущий ключ, не сравнивает
поколения ключей и не отклоняет `e2ee` только из-за старого `key_uuid`.

Только содержимое `markdown` участвует в серверном поиске упоминаний,
извлечении Markdown-ссылок на файлы и медиа и создании резюме темы. Значения
`e2ee` и `key_*` без изменений проходят через создание, историю и realtime.
Существующее состояние непрочитанных сообщений и уведомлений обновляется, но
backend не создаёт расшифрованный preview уведомления; отображаемый текст
выбирают клиенты.
...
| `payload` | объект | Да, я знаю. | Нет | Одна из шести фиксированных форм payload, описанных выше. |
