# `POST /api/workspace/v1/messenger/folder_items/{folder_item_uuid}/actions/unpin/invoke`
...
![Abfolge Diagramm](diagrams/post_folder_item_unpin.svg)

[Ausgangsgestalt , die bearbeitet werden kann PlantUML](diagrams/post_folder_item_unpin.puml)

## Zuordnung und öffentlicher Vertrag

Ein Ordnerelement des aktuellen Benutzers freischalten.
...
Die Befestigung ändert nur die persönliche Platzierung (placement) des Elements und ändert nichts
Sie können sich automatisch anmelden.
`FOLDER_ITEM` bleibt von der von der Vorker unterstützten Projektion aus
Aktiv `USER_STREAM_BINDING` und kanonisch `STREAM` mit
...
`STREAM.private = false`. Ihre Vorstellung von Lesen verwendet einfache
Indexverbindungen ohne `COUNT` während der Anfrage.

## Synchronisierter Weg API

1. Das Ordnerelement im angegebenen Bereich finden und sperren.
2. Setzen Sie `pinned_at` in `null`.
3. Ein unveränderbares Eintrag in die Outbox hinzufügen `folder_item.unpin`.
...
Die Antwort REST enthält sofort `pinned_at: null`; eingebettete read-only Snapshot-Ordner, Zähler und WebSocket-Events können bis zum Abschluss zurückbleiben `folder_projection`.
