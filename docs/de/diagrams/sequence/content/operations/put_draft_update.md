...
![Abfolge Diagramm](diagrams/put_draft_update.svg)

[Ausgangsgestalt , die bearbeitet werden kann PlantUML](diagrams/put_draft_update.puml)

## Zuordnung und öffentlicher Vertrag

Ersetzen Sie nur den Nutzen des Markdown-Berichts des Eigentümers durch Optimist Competition.
...
| --- | --- | --- |
| Weg | `draft_uuid` | UUID |
| Titel: | `If-Match` | eine obligatorische genaue, strenge Revision, z. B. `"1"` |
...
Antwortüberschrift: `ETag: "2"`.

## Fehler und Autorierung

Nur `payload` akzeptiert. Fehlende `If-Match` gibt `428` zurück. Ungültige/veraltete Revision gibt `412` mit aktuellem Entwurfsbild und ETag zurück. Ungültige Nutzlast `payload` gibt `400` zurück; nicht verfügbare Entwurf gibt nicht gefunden zurück.
...
1. Genaue Werte auflösen `If-Match`.
2. Eigentümerentwurf sperren und vergleichen `revision`.
3. Ersetzen Sie nur `payload`, vergrößern Sie `revision` und aktualisieren Sie das Timestamp.
4. Fügen Sie einen nicht veränderbaren Entwurf ohne öffentliche Ableitung in die Outbox.
5. Transaktion speichern und neue Zeile zurückgeben/ETag.

## Outbox, Typisierte Aufgaben, Worker und Echtzeitarbeit

Keine Nachrichten oder Zähler werden gezeigt.
...
Vergleichen und Aktualisieren der Revision verhindert den Verlust von Updates. Wiederholen mit dem veralteten ETag erhält `412` und kann den neueren Inhalt nicht überschreiben.
