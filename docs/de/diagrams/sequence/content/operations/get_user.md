...
![Abfolge Diagramm](diagrams/get_user.svg)

[Ausgangsgestalt , die bearbeitet werden kann PlantUML](diagrams/get_user.puml)

## Zuordnung und öffentlicher Vertrag

Erhalten Sie einen materialisierten Benutzer Workspace; die Anfrage des eigenen Benutzers aktualisiert zuerst die Projektion von IAM.
...
UUID Ein anderer Nutzer, der noch nicht durch seine eigene Aktivität materialisiert wurde, kehrt als nicht gefunden zurück.UUIDoder Zustand .IAMwird durch die Fehlergrenze verarbeitet.
...
1. Authentifizieren.
2. Wenn UUID dem aktuellen Benutzer gehört, aktualisieren Sie die Identitätsfelder von IAM synchron; ansonsten suchen Sie nur nach der Projektion.
3. Lesen und Sanitieren von Benutzern von Kanonischen Zeilen.
4. GET erstellt keine öffentlichen Outbox-Einträge, Aufgaben oder Ereignisse.
