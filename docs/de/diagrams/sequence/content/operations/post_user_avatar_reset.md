...
![Abfolge Diagramm](diagrams/post_user_avatar_reset.svg)

[Ausgangsgestalt , die bearbeitet werden kann PlantUML](diagrams/post_user_avatar_reset.puml)

## Zuordnung und öffentlicher Vertrag

Der Avatar des authentifizierten Benutzers wird auf den Standard-Avatar zurückgeführt .URNGravatar oder Sicherung.
...
Nur eigene UUID akzeptiert. Körper  leerer JSON-Objekt; Fehler IAM/Validierung werden durch die gemeinsame Grenze verarbeitet.
...
1. Überprüfen Sie die eigene UUID und berechnen Sie die kanonische URN Gravatar oder die Sicherung.
2. Benutzer sperren und nur aktualisieren `avatar`.
3. Ein unveränderbares Abstands-Avatar-Eintrag in die Outbox hinzufügen.
4. Transaktion festhalten und Benutzer zurückgeben.
5. Nach dem Verlinkungserneuerung den übertragenen Benutzeravatar aus dem Speicher löschen.

## Outbox, Typisierte Aufgaben, Worker und Echtzeitarbeit

Das fertige Benutzerbild folgt dem festgelegten kanonischen Zustand; alte Bytes können später gelöscht werden, aber der öffentliche Zugriff darauf verschwindet sofort.
...
Das wiederholte Ablenken kommt dem gleichen kanonischen URN entgegen. Das Sperren der Benutzerzeile verhindert, dass die Ablenkungs-/Aufladen-Rennen einen gemischten Link erstellen; der abschließende Avatar bestimmt die Reihenfolge der Transaktionen.
