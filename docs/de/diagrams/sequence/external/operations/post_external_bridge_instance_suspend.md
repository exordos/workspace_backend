# Das Exemplar der Außenbrücke wird ausgesetzt
...
Die Identität sofort aufzuhalten, ohne dass ihre Generation zurückruft..

![Folgebild](diagrams/post_external_bridge_instance_suspend.svg)

[Ausgangsgestalt , die bearbeitet werden kann PlantUML](diagrams/post_external_bridge_instance_suspend.puml)
...
| --- | --- |
| `403` | `workspace.external_bridge_instance.suspend` ist nicht zugelassen oder die Ressource befindet sich außerhalb des autorisierten Bereichs. |
...
2. Sperren Sie die Bridge-Instanz; Verwenden Sie den `suspend`-Zustandsschritt und die Revisions-/Generierungsregel; Schreiben Sie einen unveränderlichen Domain-Outbox-Eintrag; Verwalten Sie die Transaktion.
