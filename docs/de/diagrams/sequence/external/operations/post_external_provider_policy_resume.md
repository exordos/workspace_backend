# Wiederherstellung der Politik des externen Providers
...
Nach Überprüfung der Richtlinien die Anbieteransicht für alle Bereiche wiederherstellen.

![Folgebild](diagrams/post_external_provider_policy_resume.svg)

[Ausgangsgestalt , die bearbeitet werden kann PlantUML](diagrams/post_external_provider_policy_resume.puml)
...
| --- | --- |
| `403` | `workspace.external_provider_policy.resume` ist nicht zugelassen oder die Ressource befindet sich außerhalb des autorisierten Bereichs. |
...
2. Verhindern Sie die Politik; setzen Sie `emergency_suspended=false`; erhöhen Sie die Revision; Atomär den gewünschten Zustand und unveränderlich hinzufügen outbox.
