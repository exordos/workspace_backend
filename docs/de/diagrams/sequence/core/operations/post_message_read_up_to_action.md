...
![Abfolge Diagramm](diagrams/post_message_read_up_to_action.svg)

Ausgangsgestalt , die bearbeitet werden kann: [`post_message_read_up_to_action.puml`](diagrams/post_message_read_up_to_action.puml).

## Die Operation

**Methode und Weg:** `POST /api/workspace/v1/messenger/messages/{message_uuid}/actions/read_up_to/invoke`

**Zweck:** Die Themen des aktuellen Benutzers bis zum Ende der Benutzung als gelesen zu markieren
Grenzen der gewählten öffentlichen Placement UUID inklusive.
...
Der Weg enthält öffentliche `MESSAGE_PLACEMENT.uuid`, also spezifische Streams und
topic Sie werden auch bei mehreren Platzierungen eines
- Das ist ein Kanonische Nachricht..
...
Public `uuid` und route ID sind gleich `MESSAGE_PLACEMENT.uuid`; canonical `MESSAGE.uuid` und `binding_uuid` sind verborgen. Placement gibt eindeutig stream/topic boundary an, und action überprüft synchron active membership und generation.

## Synchrone Transaktion

1. Erlauben Sie die öffentliche Placement UUID in der Grenze
   `(MESSAGE.created_at, MESSAGE_PLACEMENT.uuid)` und der klare Kontext des Themas.
...
3. Fügen Sie für jede Ausgabe ein separates immutable outbox-Event hinzu task
   Wirkliche Lesefläche des Bandes.
