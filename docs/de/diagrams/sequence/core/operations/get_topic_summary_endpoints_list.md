...
![Abfolge Diagramm](diagrams/get_topic_summary_endpoints_list.svg)

Ausgangsgestalt , die bearbeitet werden kann: [`get_topic_summary_endpoints_list.puml`](diagrams/get_topic_summary_endpoints_list.puml).

## Die Operation

**Methode und Weg:** `GET /api/workspace/v1/messenger/topic_summary_endpoints/`

**Zweck:** Eine Liste der globalen OpenAI-kompatiblen Endpunkte der Zusammenfassungen in Prioritäts- und UUID.

## Öffentliche Anfrage

Ohne Körper; Filter der Sammlung und die Umschreibung der Ordnung werden nicht akzeptiert.

Der aktuelle Endpunkt nimmt `page_limit` nicht an und liest die gesamte Register von Endpunkten
Die Punkte  sind die Current Gap.`page_limit`und `page_marker`:
Abwesenheit/`0` gibt `100`, `1..500` wird genau angenommen, negativ,
und `>500` gibt HTTP `400` ohne Clamp. JSON bleibt ein Array;
Die Seite-Länge wird durch Standard `X-Pagination-Limit` und
`X-Pagination-Marker`, Also keine neuen JSON-Felder erscheinen.
