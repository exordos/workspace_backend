...
迁移 `0182` 会在每个域 outbox 事件所在的同一数据库事务中创建持久投影任务。
语句级触发器覆盖所有写入方，包括批量历史导入和修复写入，同时保留现有的每个
outbox 事件唯一任务标识和排序字段。迁移 `0183` 会在触发器 DDL 提交后使用单独
事务补齐已有缺口，然后 worker 才停止从历史 outbox 行派生任务。

触发器插入任务后会发送 PostgreSQL 通知。通知只负责唤醒 worker；持久任务表仍是
事实来源。短超时 polling 会继续处理重试期限，并覆盖丢失或延迟的通知。这样每次
worker 空闲循环不再执行完整的 outbox anti-join，同时仍保留重启恢复和通知丢失恢复。
