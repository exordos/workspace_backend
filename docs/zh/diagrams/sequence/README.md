[← 文件的主要索引](../../index.md) · [背景流的部分](worker_flows/README.md) · [现行 API 合同](../../workspace_api.md)

# 操作序列图 Workspace API

状态: ** 已完成的项目,开始与现行公开合同进行了对比**.

这份目录涵盖了一切.**语义上有109种不同HTTP操作的方法+路径»**:
108 控制器操作由生成的 OpenAPI 和一个可规的中间软件终点
`GET /api/workspace/v1/messenger/server_settings`. 同一个中间处理器也接受
没有重定向的,最后的斜和相同的直线`200`;这是一个名
单个操作,而不是单独的终点.
执行时间流WebSocket活动,每个人都有.HTTP- 操作存在
独立的Markdown描述,可编辑的PlantUML源代码,
在本地呈现 SVG.

设计提案文件解释了 RestAlchemy 交易,交易Outbox,任务,工人和事件的目标流,但不改变现有的公共流 API.

已接受的 target compatibility rules 对于所有操作文档:

- resource-list pagination: 没有/`0` `page_limit` => `100`, `1..500`
  确切,负/不整/`>500` => HTTP `400`,unlimited 没有;
- `2xx`/`201` 代表 commit primary mutation 和 immediate read-your-write 对于
  作者,但收件人/history/counters/snapshots/events 构成异步;
  约一秒 SLO 意图,不严格保证;
- worker 记录了 projection update 和所有相关的 durable ready event
  rows 只有一个数据库交易. at-least-once;
  reconnect 使用空格和客户端的dedupe event UUID.

完全出口的客户必须
在下一个标记没有之前阅读页面; 发布注释应该描述
它们的异步可视性.
...
| 家庭 | 覆盖面 | 索引 |
| --- | ---: | --- |
| 主要的 Messenger | 45 HTTP-操作 |  [`core/README.md`](core/README.md)  |
| 内容,用户和客户端状态 Workspace | 35 HTTP-操作 |  [`content/README.md`](content/README.md)  |
| 事件,外部整合和执行时间 | 29 HTTP-操作 + WebSocket |  [`external/README.md`](external/README.md)  |
| 工作室,Outbox和投影流 | 7 没有流动 HTTP |  [`worker_flows/README.md`](worker_flows/README.md)  |

## 完全的覆盖矩阵

### 路线索引

| 方法+路径 | Markdown 操作 | PlantUML | SVG |
| --- | --- | --- | --- |
...
| 执行时间输入点 | Markdown | PlantUML | SVG |
| --- | --- | --- | --- |
| `GET /api/workspace/v1/events/ws` (转换到 WebSocket 和执行时间流) |  [`websocket_events.md`](external/operations/websocket_events.md)  |  [`websocket_events.puml`](external/operations/diagrams/websocket_events.puml)  |  [`websocket_events.svg`](external/operations/diagrams/websocket_events.svg)  |

## 工作坊的背景流

| 流量 | Markdown | PlantUML | SVG |
| --- | --- | --- | --- |
| 工厂的架构 |  [`worker_architecture.md`](worker_flows/worker_architecture.md)  |  [`worker_architecture.puml`](worker_flows/diagrams/worker_architecture.puml)  |  [`worker_architecture.svg`](worker_flows/diagrams/worker_architecture.svg)  |
| 收件人分配 (fan-out) |  [`task_fanout.md`](worker_flows/task_fanout.md)  |  [`task_fanout.puml`](worker_flows/diagrams/task_fanout.puml)  |  [`task_fanout.svg`](worker_flows/diagrams/task_fanout.svg)  |
| 内容和提及 |  [`task_content_mentions.md`](worker_flows/task_content_mentions.md)  |  [`task_content_mentions.puml`](worker_flows/diagrams/task_content_mentions.puml)  |  [`task_content_mentions.svg`](worker_flows/diagrams/task_content_mentions.svg)  |
| 反应图片 |  [`task_reaction_snapshot.md`](worker_flows/task_reaction_snapshot.md)  |  [`task_reaction_snapshot.puml`](worker_flows/diagrams/task_reaction_snapshot.puml)  |  [`task_reaction_snapshot.svg`](worker_flows/diagrams/task_reaction_snapshot.svg)  |
| 读取和计数容器 |  [`task_read_counters.md`](worker_flows/task_read_counters.md)  |  [`task_read_counters.puml`](worker_flows/diagrams/task_read_counters.puml)  |  [`task_read_counters.svg`](worker_flows/diagrams/task_read_counters.svg)  |
| 交付照片和准备好活动 |  [`task_delivery_snapshot_event.md`](worker_flows/task_delivery_snapshot_event.md)  |  [`task_delivery_snapshot_event.puml`](worker_flows/diagrams/task_delivery_snapshot_event.puml)  |  [`task_delivery_snapshot_event.svg`](worker_flows/diagrams/task_delivery_snapshot_event.svg)  |
| 成员和主题政策的重组 |  [`task_topic_membership_policy_rebuild.md`](worker_flows/task_topic_membership_policy_rebuild.md)  |  [`task_topic_membership_policy_rebuild.puml`](worker_flows/diagrams/task_topic_membership_policy_rebuild.puml)  |  [`task_topic_membership_policy_rebuild.svg`](worker_flows/diagrams/task_topic_membership_policy_rebuild.svg)  |
| 标题的正规状态投影 |  [`task_topic_membership_policy_rebuild.md`](worker_flows/task_topic_membership_policy_rebuild.md#topic_state_projection)  |  [`task_topic_membership_policy_rebuild.puml`](worker_flows/diagrams/task_topic_membership_policy_rebuild.puml)  |  [`task_topic_membership_policy_rebuild.svg`](worker_flows/diagrams/task_topic_membership_policy_rebuild.svg)  |
| Runbook 移民和发行 |  [`migration_release_runbook.md`](worker_flows/migration_release_runbook.md)  |  [`migration_release_runbook.puml`](worker_flows/diagrams/migration_release_runbook.puml)  |  [`migration_release_runbook.svg`](worker_flows/diagrams/migration_release_runbook.svg)  |

## 导航和边界

每个 Markdown 操作,家族或工作组的导航行都在上面和底部. [`workspace_api.md`](../../workspace_api.md); 序列的规范是从文档开始的设计建议,不允许在生产代码中实现.
