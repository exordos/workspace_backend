...
![序列图](diagrams/post_external_bridge_instance_revoke.svg)

[可编辑的源 PlantUML](diagrams/post_external_bridge_instance_revoke.puml)
...
| HTTP | 公众行为 |
| --- | --- |
| `403` | 没有 `workspace.external_bridge_instance.revoke` 权限,或者资源在未经授权的区域. |
...
2. 封锁桥本;应用 `revoke`状态转换和修改/代号规则;写不变域名Outbox条目;记录交易.
