...
![序列图](diagrams/post_external_chat_deselect.svg)

[可编辑的源 PlantUML](diagrams/post_external_chat_deselect.puml)
...
2. 阻止目的;取消服务提供商的持续等待;删除数据库拥有的投影状态/生命周期访问;写出不可变的删除/更新的Outbox;记录交易.
3. 只有在交易被固定后返回回复;网络交付从来没有在交易内执行.

## 背景处理,事件和一致性

类型化任务:单独的 `topic_membership_policy_rebuild`
关联的主题和 `delivery_snapshot_event` 对于 external-chat state/event,每个
具有其源Outbox事件;客户端请求不会绕过消息或绑定.

背景处理器在一个DB交易中记录了实质化的状态和完整图像的完整封面`external_chat.deleted`;两个效果是共同提交或滚回. 在提交之后,单独的管理器WebSocket发送,重复和播放; API/worker不拥有客户端连接.

客户可见的一致性:聊天目的会立即从公众图片中删除;限制的投影/文件清理和缓存禁用可能会异步完成.
