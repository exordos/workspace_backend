...
检查提供商显示,有效的可能性和转换损失到正规的输出变化.

![序列图](diagrams/post_external_operation_preflight.svg)

[可编辑的源 PlantUML](diagrams/post_external_operation_preflight.puml)
...
类型化投影任务:没有.预先检查只执行阅读,不应排队提供商工作,Outbox记录或投影任务.

没有一个已准备的公共事件 Workspace 创建,因此单独的管理员 WebSocket 没有什么可提供.

客户可见的一致性:结果是决定在特定时刻的机会/损失. 随后的变化必须重新检查自己的交易中的授权/机会.
