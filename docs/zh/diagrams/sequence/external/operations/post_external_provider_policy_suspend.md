...
![序列图](diagrams/post_external_provider_policy_suspend.svg)

[可编辑的源 PlantUML](diagrams/post_external_provider_policy_suspend.puml)
...
| HTTP | 公众行为 |
| --- | --- |
| `403` | 没有 `workspace.external_provider_policy.suspend` 权限,或者资源在未经授权的区域. |
...
2. 封锁政策;设置`emergency_suspended=true`;扩大修改;原子添加所需状态和不变 outbox.
