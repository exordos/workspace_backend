...
![序列图](diagrams/post_external_account_disconnect.svg)

[可编辑的源 PlantUML](diagrams/post_external_account_disconnect.puml)
...
| HTTP | 公众行为 |
| --- | --- |
| `403` | 没有 `workspace.external_account.disconnect` 权限,或者资源在未经授权的区域. |
...
2. 封锁帐户;设置状态`disconnected`,没有
   已准备好进行现场运行; 取消 account lease generation, 添加关闭的状态,
   不变的更新外框; 记录交易.
...
客户端可见的一致性:关闭状态立即记录.
Credential/account, mappings, user bindings 已可见的历史记录将保存
frozen 为了重新连接;新的提供者事件不再接收.
是删除,不隐藏以前访问的历史记录.
