...
![序列图](diagrams/post_message_read_action.svg)

可编辑的源: [`post_message_read_action.puml`](diagrams/post_message_read_action.puml).

## 操作

**方法和方式:** `POST /api/workspace/v1/messenger/messages/{message_uuid}/actions/read/invoke`
...
公共`uuid`和路由ID是相同的.`MESSAGE_PLACEMENT.uuid`; canonical `MESSAGE.uuid`没有`binding_uuid`隐藏的位置.`USER_MESSAGE_STATE (project,user,placement)`并且 action 同时检查 active membership 和 generation.

## 同步交易

1. 允许公开的 UUID 放置和当前用户访问.
2. 设置一个唯一值 USER_MESSAGE_STATE.read.
