...
![序列图](diagrams/post_topic_notifications_action.svg)

可编辑的源: [`post_topic_notifications_action.puml`](diagrams/post_topic_notifications_action.puml).

## 操作

**方法和方式:** `POST /api/workspace/v1/messenger/stream_topics/{topic_uuid}/actions/notifications/invoke`
...
允许 `mute`, `default`, `follow`; `unmute` 仅允许被杀的流,否则返回 `400001006`.
...
其他 fenced owners `user-topic`/`user-stream`/`user-folder` scopes
标题工作者没有写分类行;每个人都有
outbox event 根据单独的 immutable task.

## 公共活动和 WebSocket

更新主题和当前用户流. 管理器提供固定的已准备的行.

## 具有能力,种族和可见的时间特征

唯一的连接和当前源的重复是安全的. 调用者会立即看到状态,衍生投影和事件异步.
