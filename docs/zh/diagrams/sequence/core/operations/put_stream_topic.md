...
![序列图](diagrams/put_stream_topic.svg)

可编辑的源: [`put_stream_topic.puml`](diagrams/put_stream_topic.puml).

## 操作

**方法和方式:** `PUT /api/workspace/v1/messenger/stream_topics/{topic_uuid}`
...
3. 添加单独的 immutable 输出箱事件
   `topic_state_projection`, `topic_membership_policy_rebuild` 其他
   `delivery_snapshot_event` tasks.

影响状态: TOPIC 和 transactional outbox.

## 类型化任务和后台执行器

任务: 单独 `topic_state_projection`,
`topic_membership_policy_rebuild` 和 `delivery_snapshot_event`,每个为
自己的 source outbox event.

用户/提供商的投影可以获得 immutable tasks
`user-topic`/provider scopes. Topic worker 仅提供投资和
message bindings 专题; 一个fenced owner 写道 exact key.

## 公共活动和 WebSocket

`topic.updated` 对于用户来说.

## 具有能力,种族和可见的时间特征

更新的源码是安全的. 调用者可以立即看到结果,事件异步.
