...
![序列图](diagrams/get_streams_list.svg)

可编辑的源: [`get_streams_list.puml`](diagrams/get_streams_list.puml).

## 操作

**方法和方式:** `GET /api/workspace/v1/messenger/streams/`
...
1. 扫描唯一行USER_STREAM_BINDING并添加一个可规则的STREAM,以及默认的无需主题和最后一条消息. 已准备的未读字段从绑定中取出; 查询时的聚合不执行.
2. 直接从索引表达式返回结果.
