...
![序列图](diagrams/get_stream_bindings_list.svg)

可编辑的源: [`get_stream_bindings_list.puml`](diagrams/get_stream_bindings_list.puml).

## 操作

**方法和方式:** `GET /api/workspace/v1/messenger/stream_bindings/`
...
1. 扫描观察者和项目领域中的索引行 USER_STREAM_BINDING;只连接当前的字段,不需要集成扫描.
2. 直接从索引表达式返回结果.
