...
![序列图](diagrams/put_stream.svg)

可编辑的源: [`put_stream.puml`](diagrams/put_stream.puml).

## 操作

**方法和方式:** `PUT /api/workspace/v1/messenger/streams/{stream_uuid}`
...
3. 添加单独的 immutable 输出箱事件
   `topic_membership_policy_rebuild`, `folder_projection` 其他
...
任务:单独的 `topic_membership_policy_rebuild`, `folder_projection` 和
`delivery_snapshot_event`, 每个人都为自己的 source outbox event.

用户和提供商的投影得到了单独的 immutable tasks
实际的 `user-stream`/provider scopes.
影响到的投放/bindings;一个fenced owner写道 exact key.

## 公共活动和 WebSocket

`stream.updated` 通过控制器.

## 具有能力,种族和可见的时间特征

单一的规范行和相关源的任务防止过时的副本.调用者立即看到更新,其余的投影和事件异步.
