...
![序列图](diagrams/post_folder_item_unpin.svg)

[可编辑的源 PlantUML](diagrams/post_folder_item_unpin.puml)
...
解锁只改变元素的个人位置层 (placement),而不会改变
系统文件规则或自动会员.
`FOLDER_ITEM` 仍然是从重建的投影中维护的
活跃的 `USER_STREAM_BINDING` 和正规的 `STREAM`
`is_archived = false`: `All chats` 包括所有可用的流,
`Personal` — 只有流量从 `STREAM.private = true`, `Channels` 只有从
`STREAM.private = false`. 她的阅读方式使用了简单的
在查询时没有 `COUNT` 的索引连接.

## 同步路径 API

1. 在指定区域中查找和锁定文件元素.
2. 设置 `pinned_at` 在 `null`.
3. 添加一个不变的记录到Outbox `folder_item.unpin`.
4. 记录交易并返回更新的元素.
...
答案 REST 立即包含 `pinned_at: null`; 插入的仅读快照文件,计数器和 WebSocket 事件可能会滞后到完成 `folder_projection`.
