...
![序列图](diagrams/post_user_presence.svg)

[可编辑的源 PlantUML](diagrams/post_user_presence.puml)
...
只有本人UUID认证用户的认证. `status`接受`active|idle|offline|do_not_disturb`;`emoji`和`text`可以被省略以保持原来的值,或者传输为`null`以清理.
...
1. 检查自己的 UUID.
2. 检查 `status` 和选项.
3. 更新`status`字段并设置 `last_ping_at=now`.
4. 添加一个不变的记录到Outbox `user.presence_changed`.
5. 记录交易并恢复完整的用户图片.

## Outbox, 典型任务,工作和实时工作

API 不能同时更新每个接收者的事件. 转换不活跃的用户到`offline`  工作人员的单独责任.
...
用户的正规行序列化竞争的存在记录;击败了最后一个固定值`status`.每个outbox事件输出一个单独的 immutable task;一个immutable处理器读取当前的初始状态,因此重复相同的 task不会把过时的图片输出为当前的.

对于 mapped Zulip identity Bridge,它连续传递Workspace-origin 和
Zulip-origin presence/status changes. 最后确认的更改胜利;
`origin`/`causation_uuid` 它们只会压制自己的回声,
系统. 公共请求/response shape 不变.
