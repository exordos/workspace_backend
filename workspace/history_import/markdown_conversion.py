...
class MarkdownLink:
    ...
    def with_destination(self, destination: str) -> str:
        if self.reference:
            ...
            return f"{marker}[{self.label}]({destination}{self.destination_suffix})"
...
def _inline_link_details(
    ...
    *,
    ...
    if destination_start < destination_end and state.src[destination_start] == "<":
        destination_start += 1
        destination_end -= 1
    details.update(
        {
...
def _annotate_inline_rule(
    ...
    def annotated(state: rules_inline.StateInline, silent: bool) -> bool:
        ...
        candidates = [
            token for token in state.tokens[token_count:] if token.type == token_type
        ]
        if not candidates:
            return matched
        token = candidates[0]
        if name in {"link", "image"}:
            token.meta.update(_inline_link_details(state, start, image=name == "image"))
        else:
            token.meta.update(
                {
                    "source_start": start,
...
def _commonmark() -> markdown_it.MarkdownIt:
    markdown = markdown_it.MarkdownIt("commonmark", {"store_labels": True})
...
def _parse(
    content: str,
) -> tuple[list[markdown_token.Token], dict[str, object], _Source]:
...
def _inline_links(
    inline: markdown_token.Token,
    source: _Source,
) -> list[tuple[markdown_token.Token, MarkdownLink, tuple[int, int]]]:
    boundaries = _inline_source_boundaries(inline, source)
    links: list[tuple[markdown_token.Token, MarkdownLink, tuple[int, int]]] = []
...
def _standalone_link(
    ...
    outside = list(children[:opening]) + list(children[closing:])
    if any(token.type != "text" or token.content.strip() for token in outside):
        return None
    return links[0][1], links[0][2]


def _quote_fence(token: markdown_token.Token) -> bool:
    return (
        token.type == "fence"
        ...
        for candidate in reversed(tokens[:fence_index]):
            ...
            for _token, link, span in reversed(candidate_links):
                ...
                if replacement is not None:
                    ...
                    closing_ending = _split_line_ending(source.lines[fence.map[1] - 1])[
                        1
                    ]
                    edits.append(
                        _Edit(
    ...
    *,
    ...
    for token in tokens:
        ...
        for link_index in sorted(
            ...
            key=lambda index: token_links[index][2][1] - token_links[index][2][0],
        ):
            ...
            if link.reference and isinstance(label, str):
                reference_results.setdefault(label, []).append(replacement != link.raw)

        for child in token.children or ():
            if child.type not in {"code_inline", "html_inline"}:
                continue
            code_span = _absolute_span(
                boundaries,
                child.meta.get("source_start"),
                child.meta.get("source_end"),
            )
            if code_span is not None:
                edits.append(_Edit(code_span[0], code_span[1], None, 70))
    ...
    if isinstance(references, dict):
        for label, record in references.items():
            ...
            if results and all(results) and label not in literal_reference_labels:
