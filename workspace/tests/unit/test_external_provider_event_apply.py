...
def test_partial_move_explicit_link_is_not_replaced(partial_move, incoming):
    event, session, identity, _existing, saved, _broadcasts = partial_move
...
def test_partial_move_does_not_copy_link_from_unproven_identity(
    partial_move,
    conflict,
):
    event, session, identity, existing, saved, _broadcasts = partial_move
    if conflict == "account":
