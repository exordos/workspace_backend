...
CREATE TEMP TABLE messenger_v2_provider_participant_repair_targets (
    ...
    PRIMARY KEY (project_id, user_uuid, placement_uuid)
) ON COMMIT DROP;

INSERT INTO messenger_v2_provider_participant_repair_targets (
    project_id, user_uuid, legacy_message_uuid, placement_uuid,
    stream_uuid, topic_uuid, membership_generation, author_uuid,
    ingest_sequence, effective_read, mentioned, created_at, updated_at
)
SELECT event.project_id, access.user_uuid, legacy.uuid, placement.uuid,
       placement.stream_uuid, placement.topic_uuid,
       stream_binding.membership_generation, canonical.author_uuid,
       legacy.ingest_sequence,
       existing_state.read_at IS NOT NULL
           OR CASE
           ...
           END,
       POSITION(
           '](urn:user:' || lower(access.user_uuid::text) || ')'
...
JOIN messenger_message_placements AS placement
 ...
 AND COALESCE(placement.legacy_public_uuid, placement.uuid) = legacy.uuid
...
JOIN m_confirmed_external_stream_access AS access
  ON access.project_id = placement.project_id
 AND access.stream_uuid = placement.stream_uuid
JOIN messenger_stream_bindings AS stream_binding
 ...
 AND stream_binding.user_uuid = access.user_uuid
...
LEFT JOIN m_workspace_user_message_flags AS legacy_flag
 ...
 AND legacy_flag.user_uuid = access.user_uuid
 AND legacy_flag.uuid = legacy.uuid
LEFT JOIN m_workspace_user_read_chunks_v1 AS read_chunk
  ON read_chunk.user_uuid = access.user_uuid
 AND read_chunk.chunk_number = legacy.ingest_sequence / 4096
LEFT JOIN messenger_user_message_states AS existing_state
  ON existing_state.project_id = placement.project_id
 AND existing_state.placement_uuid = placement.uuid
 AND existing_state.user_uuid = access.user_uuid
 AND existing_state.membership_generation =
...
CREATE INDEX messenger_v2_provider_participant_repair_stream_idx
ON messenger_v2_provider_participant_repair_targets (
    project_id, user_uuid, stream_uuid
);

CREATE INDEX messenger_v2_provider_participant_repair_topic_idx
ON messenger_v2_provider_participant_repair_targets (
    project_id, user_uuid, topic_uuid
);

ANALYZE messenger_v2_provider_participant_repair_targets;
...
SELECT messenger_uuid_v5(target.placement_uuid, target.user_uuid::text),
       target.project_id, target.placement_uuid, target.user_uuid,
...
FROM messenger_v2_provider_participant_repair_targets AS target
...
CREATE TEMP TABLE messenger_v2_provider_participant_repair_scopes
ON COMMIT DROP AS
SELECT DISTINCT project_id, user_uuid, stream_uuid
FROM messenger_v2_provider_participant_repair_targets;

CREATE UNIQUE INDEX messenger_v2_provider_participant_repair_scope_idx
ON messenger_v2_provider_participant_repair_scopes (
    project_id, user_uuid, stream_uuid
);

ANALYZE messenger_v2_provider_participant_repair_scopes;

CREATE TEMP TABLE messenger_v2_provider_participant_repair_topic_scopes
ON COMMIT DROP AS
SELECT DISTINCT project_id, user_uuid, stream_uuid, topic_uuid
FROM messenger_v2_provider_participant_repair_targets;

CREATE UNIQUE INDEX messenger_v2_provider_participant_repair_topic_scope_idx
ON messenger_v2_provider_participant_repair_topic_scopes (
    project_id, user_uuid, topic_uuid
);

ANALYZE messenger_v2_provider_participant_repair_topic_scopes;
...
WITH snapshots AS MATERIALIZED (
    ...
    JOIN messenger_v2_provider_participant_repair_scopes AS scope
    ...
    WHERE target.active AND message.uuid IS NOT NULL
    GROUP BY target.project_id, target.user_uuid, target.stream_uuid
)
...
WITH snapshots AS MATERIALIZED (
    ...
    JOIN messenger_v2_provider_participant_repair_topic_scopes AS scope
      ON scope.project_id = target.project_id
     AND scope.user_uuid = target.user_uuid
     AND scope.topic_uuid = target.topic_uuid
    ...
    GROUP BY target.project_id, target.user_uuid, target.topic_uuid
)
...
WITH affected_users AS MATERIALIZED (
    ...
    FROM messenger_v2_provider_participant_repair_scopes
...
DO $provider_participant_repair_guard$
BEGIN
    IF EXISTS (
        SELECT 1
        FROM messenger_v2_provider_participant_repair_targets AS target
        LEFT JOIN messenger_user_message_bindings AS binding
         ...
         AND binding.membership_generation = target.membership_generation
        LEFT JOIN messenger_user_message_states AS state
          ON state.project_id = target.project_id
         AND state.placement_uuid = target.placement_uuid
         AND state.user_uuid = target.user_uuid
        ...
        WHERE binding.uuid IS NULL OR state.uuid IS NULL
           OR (state.read_at IS NOT NULL) IS DISTINCT FROM target.effective_read
           OR (
        ...
        RAISE EXCEPTION
            'Provider participant message state repair is incomplete';
    END IF;
END;
$provider_participant_repair_guard$;
"""


class MigrationStep(migrations.AbstractMigrationStep):
    def __init__(self):
        self._depends = ["0164-stabilize-provider-private-chat-labels-f8bd03.py"]

    @property
    def migration_id(self):
        return "73514c7e-4cd6-496a-be60-1896107e7087"
